pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                sh 'GrandMaster=$(aws ec2 describe-instances --filters Name=tag:Name,Values="Docker Control Machine"  Name=instance-state-name,Values=running --region "us-east-1" --query "Reservations[].Instances[].InstanceId" --output text) && Test=$(/usr/local/bin/mssh $GrandMaster --region us-east-1 -oStrictHostKeyChecking=no "aws ec2 describe-instances")'
