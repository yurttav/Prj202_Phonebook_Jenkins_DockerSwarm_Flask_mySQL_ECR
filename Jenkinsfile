    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                ...
                sh 'mssh $GrandMaster --region us-east-1 -oStrictHostKeyChecking=no aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin "$ECR_REGISTRY" && docker pull "$ECR_REGISTRY/$APP_REPO_NAME:latest" && docker stack deploy -c docker-compose.yml phonebook'
            }
        }
